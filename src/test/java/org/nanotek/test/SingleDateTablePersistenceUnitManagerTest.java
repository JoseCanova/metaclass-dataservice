...
public class SingleDateTablePersistenceUnitManagerTest {
	...
	public SingleDateTablePersistenceUnitManagerTest() {
	}
	...
	void testEntityManagerInitialization() throws ClassNotFoundException {
		...
		Query query = rm.createQuery("select a from org.nanotek.data.SimpleDateTable a");
		List<?> resultList = query.getResultList();
		assertNotNull(resultList);
		assertNotNull(classLoader);
		executeInsert(rm);
		Query query1 = rm.createQuery("select a from org.nanotek.data.SimpleDateTable a");
		List<?> resultList1 = query1.getResultList();
		assertTrue(resultList1.size()==1);
		executeDelete(rm);
		Query query11 = rm.createQuery("select a from org.nanotek.data.SimpleDateTable a");
	...
	private void executeDelete(EntityManager rm) {
        ...
        Query query11= rm.createQuery("delete from org.nanotek.data.SimpleDateTable a");
        tx.begin();
	...
	private void executeInsert(EntityManager rm) throws ClassNotFoundException {
        EntityTransaction tx = rm.getTransaction();
        tx.begin();
		Class<?> clazz = Class.forName("org.nanotek.data.SimpleDateTable" , false , classLoader);
